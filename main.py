...
from webserver import BotController
...
class MyBot(BaseBot):
    ...
    async def dashboard_integration_loop(self):
        """🌐 حلقة التحكم من Dashboard - تطبيق الملابس والأوامر"""
        error_count = 0
        try:
            while True:
                try:
                    ...
                    if pending_item_id:
                        try:
                            ...
                            outfit_items = []
                            if current_outfit_response and current_outfit_response.outfit:
            ...
            print(f"❌ Dashboard loop critical error: {e}")

    async def dashboard_integration_loop_commands(self):
        """تطبيق الأوامر - حلقة منفصلة للأمان"""
        try:
            while True:
                try:
                    # تحقق من الأوامر المعلقة من Dashboard
                    ...
                    print("🛑 توقفت حلقة Dashboard")
                    break
                except Exception as e:
                    print(f"⚠️ خطأ في حلقة Dashboard: {e}")
                    await asyncio.sleep(1)
        except Exception as e:
            print(f"❌ خطأ في dashboard_integration_loop: {e}")
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        ...
        await self.highrise.chat("✅ البوت بدأ التشغيل وسيبدأ الرقص الآن! 🎭")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("add") and message.lower().endswith("mod"):
            ...
            try:
                ...
                self.mod_users.add(target_username)  # Track mod user
        ...
        if message.lower().startswith("rem") and message.lower().endswith("mod"):
            ...
            try:
                ...
                if target_username in self.mod_users:
                    self.mod_users.remove(target_username)  # Remove from tracked mods
    ...
    async def run(self):
        ...
        "bb3c367414a98243c66efd63d24501aa78ad4d25e131423fc731e4e7a8b1c26e"
            )
        ]
...
if __name__ == "__main__":
        ...
        # 🌐 تشغيل السيرفر في خيط منفصل (مش main thread!)
        server_thread = Thread(target=keep_alive, daemon=True)
        server_thread.start()
        print("✅ Dashboard server started in background thread")
        time.sleep(1)  # انتظر السيرفر يبدأ
        
        bot_instance = MyBot()
        load_custom_welcomes()
        
        # 🔄 حلقة الاتصال - إعادة اتصال فورية بدون انتظار طويل
        retry_count = 0
        consecutive_fails = 0
        
        print("🚀 🚀 Starting Highrise Bot - ZERO DOWNTIME MODE")
        print("📦 ZERO DOWNTIME MODE Active")
        ...
        while True:
            try:
                ...
                # ⚡ إعادة اتصال فورية جداً - 0.5 ثانية فقط!
                wait_time = 0.5 if consecutive_fails < 3 else min(1.0 + (consecutive_fails * 0.2), 5.0)
                
                print(f"❌ Connection lost (attempt {retry_count}) - Error: {str(error_msg)[:50]}")
                print(f"⚡ Instant reconnect in {wait_time:.1f}s...")
                
                time.sleep(wait_time)
                
                # ♻️ إعادة تهيئة بعد 10 فشل متتالي
                if consecutive_fails >= 10:
                    print("🔄 Reinitializing after 10 failures...")
