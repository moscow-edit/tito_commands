...
from webserver import BotController, keep_alive
...
class MyBot(BaseBot):
    ...
    async def run(self):
        ...
        definitions = [
            HighriseBotDefinition(
                self,
                "678a1472765cf0644e8bcb89",
                "10ef90feb20554fa486d606f40d708487d0e2cd3c5669829cc5fe8b236eb5187"
            )
        ]
    ...
    async def update_room_admins(self):
        try:
            ...
            if not hasattr(users_data_response, 'content'):
                return
    ...
    async def equip_user(self, user: User, message: str):
        try:
            ...
            try:
                ...
                room_users = room_users_resp.content if hasattr(room_users_resp, 'content') else []
    ...
    async def get_bot_gold(self):
        try:
            ...
            wallet = wallet_resp.content if hasattr(wallet_resp, 'content') else []
    ...
    async def check_dance_area(self):
        ...
        while True:  # 🛡️ Outer wrapper
            try:
                ...
                crash_count += 1
                print(f"⚠️ check_dance_area crashed #{crash_count}: {str(e)[:40]} - سيحاول مرة أخرى...")
                # ♾️ يحاول لا نهائياً - لا يتوقف أبداً
                sleep_time = min(2 * crash_count, 30)
                await asyncio.sleep(sleep_time)
    ...
    async def on_message(self, user_id: str, conversation_id: str, is_new_conversation: bool) -> None:
        try:
            ...
            if not (response and hasattr(response, 'messages') and response.messages and len(response.messages) > 0):
                return
            
            message = response.messages[0].content.strip() if hasattr(response.messages[0], 'content') else ""
...
if __name__ == "__main__":
    ...
    server_thread.start()
    print("✅ Dashboard server started in background thread")
    time.sleep(1)
    ...
    print("🚀 🚀 Starting Highrise Bot - ULTRA STABLE MODE")
    print("📦 ULTRA STABLE MODE Active - البوت لن يفصل من الروم!")
    ...
    while True:
        try:
            ...
            print(f"⚠️ Connection issue (attempt {retry_count}) - {error_msg[:60]}")

            wait_time = 0.1 if consecutive_fails < 5 else min(0.2 + (consecutive_fails * 0.1), 2.0)
            print(f"⚡ Ultra-fast reconnect in {wait_time:.2f}s...")

            time.sleep(wait_time)

            if consecutive_fails >= 20:
                print("🔄 Soft reinit after 20 attempts...")
                bot_instance = MyBot()
                consecutive_fails = 0
