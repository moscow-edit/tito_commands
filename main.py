...
server_started_dt = datetime.now()
...
class BotController:
    ...
    def get_outfits():
        try:
            if os.path.exists("data/outfits.json"):
                with open("data/outfits.json", "r", encoding="utf-8") as f:
                    return json.load(f)
...
def get_bot_status_info():
    ...
    return {
        ...
        "timestamp": time.time(),
        "server_start": server_started_dt.isoformat()
    }

@app.route("/", methods=["GET"])
def home():
    metrics.add_request()
    bot_status = get_bot_status_info()
    outfits = BotController.get_outfits()
    outfit_options = "".join([f'<option value="{oid}">{oid}</option>' for oid in outfits.keys()])
    
    # Load HTML from separate file if possible, else use template string
    try:
        with open("dashboard.html", "r", encoding="utf-8") as f:
            html = f.read()
            html = html.replace("{bot_status['uptime_formatted']}", bot_status['uptime_formatted'])
            html = html.replace("{metrics.metrics['current_memory']:.1f}", f"{metrics.metrics['current_memory']:.1f}")
            html = html.replace("{metrics.metrics['requests_count']}", str(metrics.metrics['requests_count']))
            html = html.replace("{outfit_options}", outfit_options)
            return html
    except Exception:
        return "Dashboard Load Error"

@app.route("/api/outfit/apply", methods=["POST"])
def api_apply_outfit():
    metrics.add_request()
    try:
...
def api_get_items():
    metrics.add_request()
    try:
        if os.path.exists("data/outfit_items_database.json"):
            with open("data/outfit_items_database.json", "r", encoding="utf-8") as f:
                data = json.load(f)
...
def api_create_outfit():
    ...
    try:
        ...
        items_data = [{"id": item_id, "type": "clothing", "amount": 1} for item_id in items_ids]
        if os.path.exists("data/outfits.json"):
            with open("data/outfits.json", "r", encoding="utf-8") as f:
                outfits = json.load(f)
        else:
        ...
        with open("data/outfits.json", "w", encoding="utf-8") as f:
...
def get_bot_status():
    """الحصول على حالة البوت الحالية للوحة التحكم"""
    metrics.update_memory()
    metrics.add_request()
    status = get_bot_status_info()
    return jsonify({
        "status": "connected",
        "uptime": status,
        "memory_mb": metrics.metrics["current_memory"],
        "requests": metrics.metrics["requests_count"]
    })

@app.route("/api/outfit/saved-list", methods=["GET"])
def get_saved_outfits():
    """قائمة اللبسات المحفوظة"""
    metrics.add_request()
    try:
        if os.path.exists("data/outfits.json"):
            with open("data/outfits.json", "r", encoding="utf-8") as f:
...
# 🌐 السيرفر الجانبي (لو عندك keep_alive)
...
# 🛡️ نظام الحماية الشاملة من الانقطاع - INFINITE RETRIES ♾️
class LoopProtection:
    """حماية شاملة لجميع الحلقات والعمليات غير المتزامنة - البوت لن يتوقف أبداً"""
    ...
    async def protected_loop(self, loop_func):
        """تشغيل أي حلقة بحماية كاملة - يحاول لا نهائياً"""
        ...
        while self.is_running:
            try:
                ...
                print(f"🛑 {self.name} تم إيقافه بأمان")
                self.is_running = False
                break
            except Exception as e:
                self.crash_count += 1
                self.last_error = str(e)
                print(f"⚠️ {self.name} crashed #{self.crash_count}: {str(e)[:60]} - سيحاول مرة أخرى...")
                
                # انتظار تدريجي مع cap على 30 ثانية
                sleep_time = min(2 * self.crash_count, 30)
                print(f"⏳ انتظار {sleep_time} ثانية قبل المحاولة التالية...")
                await asyncio.sleep(sleep_time)
                # ♾️ يحاول لا نهائياً - لا يتوقف أبداً
...
def save_bot_position(position: dict):
    with open("data/bot_position.json", "w", encoding="utf-8") as f:
        json.dump(position, f)

def load_bot_position():
    if os.path.exists("data/bot_position.json"):
        with open("data/bot_position.json", "r", encoding="utf-8") as f:
            return json.load(f)
    return None
...
def save_allowed_vip_users():
    with open("data/allowed_vip_users.json", "w", encoding="utf-8") as f:
        json.dump(allowed_vip_users, f)

def load_allowed_vip_users():
    global allowed_vip_users
    try:
        with open("data/allowed_vip_users.json", "r", encoding="utf-8") as f:
...
ALLOWED_SWMS_FILE = "data/allowed_swms.json"
...
def load_custom_welcomes():
    try:
        with open("data/special_welcomes.json", "r", encoding="utf-8") as f:
            data = json.load(f)
...
def save_custom_welcomes():
    with open("data/special_welcomes.json", "w", encoding="utf-8") as f:
        json.dump(special_welcomes, f, ensure_ascii=False, indent=2)


room_commands_file = "data/room_commands.json"
room_commands = {}
...
def load_owners():
    global owners
    if os.path.exists("data/owners.json"):
        with open("data/owners.json", "r", encoding="utf-8") as f:
            try:
                data = json.load(f)
...
def save_owners():
    with open("data/owners.json", "w", encoding="utf-8") as f:
...
def load_vip_users():
    global vip_users
    if os.path.exists("data/vip_users.json"):
        with open("data/vip_users.json", "r", encoding="utf-8") as f:
            try:
...
def save_vip_users():
    with open("data/vip_users.json", "w", encoding="utf-8") as f:
...
class MyBot(BaseBot):
    ...
    async def run(self, room_id, api_token):
        """إصلاح مشكلة الـ attribute 'run' عبر توفير الدالة المطلوبة"""
        from highrise.__main__ import BotDefinition as HighriseBotDefinition, main as highrise_main
        
        retry_count = 0
        while True:
            try:
                definitions = [HighriseBotDefinition(self, room_id, api_token)]
                print(f"🚀 Connecting to Highrise (attempt {retry_count + 1})...")
                await highrise_main(definitions)
                retry_count = 0 
            except Exception as e:
                retry_count += 1
                delay = min(5 * (2 ** min(retry_count - 1, 2)), 30)
    ...
    def get_content(self, response, default=[]):
        """دالة موحدة للحصول على المحتوى بشكل آمن من ردود المكتبة"""
        if response is None:
            return default
    ...
    def __init__(self):
        ...
        self.teleport_roles = {}
        self.teleport_file = "data/teleports.json"
        ...
        self.last_welcome_time = 0  # 🕒 منع تكرار ترحيب العام بسرعة
        ...
        # 👮 إدارة المشرفين
        self.admins = []
        self.filename = "data/admins.json"
        self.load_admins()
        
        # 💰 إعداد صلاحيات الأوتو تيب
        self.auto_tip_enabled = False
        self.file_path = "data/allowed_autotip_users.json"
    ...
    async def get_user_id_by_username(self, username: str) -> str | None:
            """🔍 جلب user_id من username باستخدام Web API"""
            import requests
            
            # أولاً: نشوف الـ cache
            cache = self.load_user_cache()
            if username.lower() in cache:
                print(f"✅ Cache: Found {username} → {cache[username.lower()]}")
                return cache[username.lower()]
            
            # ثانياً: نستخدم create.highrise.game API (الصحيح)
            try:
                ...
                if response.status_code == 200:
                    ...
                    if users_list and len(users_list) > 0:
                        ...
                        if user_id:
                            ...
                            print(f"✅ Create API: Found {real_username} → {user_id}")
                            return user_id
                    else:
                        print(f"⚠️ Create API: User '{username}' not found")
                else:
                    print(f"⚠️ Create API error: {response.status_code}")
            except Exception as e:
                print(f"⚠️ Create API request failed: {e}")
    ...
    async def is_user_allowed(self, user: User):
        try:
            # 🤖 لو البوت نفسه (للأوامر من Dashboard)
            if hasattr(self, 'bot_username') and user.username.lower() == self.bot_username.lower():
                return True
            if hasattr(self, 'my_user_id') and user.id == self.my_user_id:
                return True
    ...
    async def update_room_admins(self):
        try:
            ...
            users_data = self.get_content(users_data_response)
            if not isinstance(users_data, list):
                print(f"⚠️ استجابة get_room_users غير صالحة أو فارغة")
                return
    ...
    async def equip_user(self, user: User, message: str):
        try:
            parts = message.split(" ")
            if len(parts) < 2:
                await self.highrise.chat("اكتب: equip @username")
                return
            ...
            # أولاً: نحاول نلاقي المستخدم في الغرفة
            try:
                room_users_resp = await self.highrise.get_room_users()
                room_users = self.get_content(room_users_resp)
                for u, pos in room_users:
                    if u.username.lower() == target_username.lower():
                        target_id = u.id
                        break
            except Exception as e:
                print(f"⚠️ خطأ في البحث في الغرفة: {e}")

            # ثانياً: لو مش موجود في الغرفة، نستخدم الـ API
            if target_id is None:
                await self.highrise.chat(f"🔍 بدور على @{target_username} برا الروم...")
                target_id = await self.get_user_id_by_username(target_username)
                
                if target_id is None:
                    await self.highrise.chat(f"❌ مش لاقي المستخدم @{target_username}")
                    return

            # جلب اللبس عن طريق Web API (شغال للي برا الروم)
            try:
                import requests
                ...
                if response.status_code == 200:
                    ...
                    if data.get("user") and data["user"].get("outfit"):
                        ...
                        # تحويل الـ outfit من API لـ Item objects
                        target_outfit = []
                ...
                print(f"⚠️ خطأ في Web API: {e}")
            
            # لو فشل الـ Web API، نحاول بالطريقة العادية
            if not target_outfit:
                try:
                    outfit_response = await self.highrise.get_user_outfit(target_id)
                    target_outfit = outfit_response.outfit if hasattr(outfit_response, 'outfit') else None
                except Exception as e:
                    print(f"⚠️ خطأ في get_user_outfit: {e}")
                    await self.highrise.chat("❌ مش قادر أجيب لبس المستخدم ده")
                    return

            if not target_outfit:
                await self.highrise.chat("❌ مش لاقي لبس للمستخدم ده")
                return

            # تطبيق اللبس على البوت
            try:
                print(f"🔄 Applying outfit with {len(target_outfit)} items...")
                await self.highrise.set_outfit(target_outfit)
                print(f"✅ Outfit applied successfully!")
                await self.highrise.chat(f"✅ تم نسخ لبس @{target_username} ولَبسه للبوت!")
            except Exception as outfit_error:
                error_msg = str(outfit_error)
                print(f"❌ خطأ في تطبيق اللبس: {error_msg}")
                if "not owned" in error_msg.lower() or "item" in error_msg.lower():
                    await self.highrise.chat(f"⚠️ البوت مش بيملك بعض القطع في لبس @{target_username}")
                else:
                    await self.highrise.chat(f"❌ خطأ في لبس الاوتفت: {error_msg[:100]}")
            
        except Exception as e:
            print(f"❌ خطأ في equip: {e}")
            await self.highrise.chat("❌ حصل خطأ في تنفيذ الأمر")
    ...
    async def get_bot_gold(self):
        try:
            wallet_resp = await self.highrise.get_wallet()
            wallet = self.get_content(wallet_resp)
    ...
    async def auto_tip_loop(self):
        """🛡️ أوتو تيب مع حماية كاملة من الانقطاع"""
        global auto_tip_enabled, auto_tip_amount, auto_tip_delay
        
        crash_count = 0
        while True:  # 🛡️ Outer wrapper - أي exception = restart
            try:
                error_count = 0
                while auto_tip_enabled:
                    try:
                        ...
                        try:
                            ...
                            room_users = self.get_content(room_users_response)
                        except Exception:
                        ...
                        print(f"⚠️ خطأ في auto_tip_loop (inner): {str(e)[:50]}")
                        await asyncio.sleep(2)

                    await asyncio.sleep(auto_tip_delay)
                
                # لو خرج من while auto_tip_enabled (disabled)
                # لا نخرج - نحتفظ بالحلقة ننتظر لما يعود auto_tip_enabled = True
                if not auto_tip_enabled:
                    await asyncio.sleep(1)  # انتظر 1 ثانية قبل المحاولة مرة أخرى
                    
            except asyncio.CancelledError:
                print("🛑 auto_tip_loop cancelled (outer)")
                # ♾️ ما نوقف - نحاول دايماً
            except Exception as e:
                crash_count += 1
                print(f"⚠️ auto_tip_loop crashed #{crash_count}: {str(e)[:50]} - سيحاول مرة أخرى...")
                # ♾️ يحاول لا نهائياً - لا يتوقف أبداً
                sleep_time = min(3 * crash_count, 30)
                await asyncio.sleep(sleep_time)
    ...
    async def ad_loop(self):
        ...
        MESSAGE = ["<#FF69B4>ارحبو حبايبي منورين روم المنتزه  🇪🇬 🦋"]
        crash_count = 0
        
        # 🛡️ منع تشغيل أكثر من نسخة من الحلقة
        if hasattr(self, '_ad_loop_running') and self._ad_loop_running:
            print("⚠️ ad_loop is already running, skipping start.")
            return
        self._ad_loop_running = True
        ...
        try:
            while True:  # 🛡️ Outer wrapper
                try:
                    ...
                    crash_count += 1
                    print(f"⚠️ ad_loop crashed #{crash_count}: {str(e)[:40]} - سيحاول مرة أخرى...")
                    sleep_time = min(2 * crash_count, 30)
                    await asyncio.sleep(sleep_time)
        finally:
            self._ad_loop_running = False

    async def check_dance_area(self):
        """📍 مراقبة منطقة الرقص - مع حماية من الانقطاع"""
        pass
    ...
    async def dashboard_integration_loop(self):
        """🌐 حلقة التحكم من Dashboard - مع حماية من الانقطاع"""
        crash_count = 0
        ...
        while True:  # 🛡️ Outer wrapper
            try:
                ...
                while True:
                    try:
                        # 🔧 أولاً: تحقق من item ID المباشر (الأولوية)
                        pending_item_id = BotController.get_pending_item()
                        if pending_item_id:
                            try:
                                # جلب الـ outfit الحالي للبوت
                                current_outfit_response = await self.highrise.get_user_outfit(self.my_user_id)
                                
                                outfit_items = []
                                if current_outfit_response and hasattr(current_outfit_response, 'outfit') and current_outfit_response.outfit:
                                    # إضافة الـ item الجديد للـ outfit الحالي
                                    outfit_items = list(current_outfit_response.outfit)
                                
                                # إضافة/استبدال الـ item الجديد
                                new_item = Item(
                                    id=pending_item_id,
                                    type='clothing',
                                    amount=1
                                ...
                                print(f"✅ تم لبس item من Dashboard: {pending_item_id}")
                                error_count = 0
                            except Exception as e:
                                error_count += 1
                                print(f"❌ خطأ في تطبيق item [{pending_item_id}]: {str(e)[:50]}")
                                if error_count > 3:
                                    print(f"❌ تطبيق item فشل عدة مرات - قد الـ item غير موجود")
                        
                        # تحقق من الملابس المعلقة من Dashboard
                        pending_outfit_id = BotController.get_pending_outfit()
                        if pending_outfit_id:
                            try:
                                outfits = BotController.get_outfits()
                                if pending_outfit_id in outfits:
                                    items_data = outfits[pending_outfit_id]
                                    outfit_items = []
                                    ...
                                    if outfit_items:
                                        await self.highrise.set_outfit(outfit=outfit_items)
                                        print(f"✅ تم تطبيق ملابس من Dashboard: {pending_outfit_id}")
                                        error_count = 0
                            except Exception as e:
                                error_count += 1
                                if error_count <= 3:
                                    print(f"⚠️ خطأ في تطبيق الملابس ({error_count}/3): {str(e)[:40]}")
                                else:
                                    print(f"❌ تطبيق الملابس فشل عدة مرات")
                        
                        # تحقق من الأوامر المعلقة من Dashboard
                        queued_cmd = BotController.get_queued_command()
                        if queued_cmd:
                            try:
                                await self.on_chat(User(id=self.my_user_id, username=self.bot_username), queued_cmd)
                                print(f"✅ تم تنفيذ أمر من Dashboard: {queued_cmd}")
                            except Exception as e:
                                print(f"⚠️ خطأ في تنفيذ الأمر: {str(e)[:40]}")
                        
                    except asyncio.CancelledError:
                        raise
                    except Exception as e:
                        print(f"⚠️ Dashboard loop error: {str(e)[:40]}")
                        await asyncio.sleep(0.5)
                    
                    await asyncio.sleep(1)  # تحقق كل ثانية
            except asyncio.CancelledError:
                print("🛑 Dashboard integration loop cancelled")
                # ♾️ ما نوقف - نحاول دايماً
                await asyncio.sleep(1)
            except Exception as e:
                crash_count += 1
                print(f"⚠️ dashboard_integration_loop crashed #{crash_count}: {str(e)[:40]} - سيحاول مرة أخرى...")
                # ♾️ يحاول لا نهائياً - لا يتوقف أبداً
                sleep_time = min(2 * crash_count, 30)
                await asyncio.sleep(sleep_time)

    async def dashboard_integration_loop_commands(self):
        """تطبيق الأوامر - حلقة منفصلة للأمان - مع حماية"""
        crash_count = 0
            ...
            try:
                while True:
                    try:
                        ...
                        if queued_cmd:
                            try:
                                # إرسال الأمر كرسالة من البوت نفسه
                                await self.on_chat(User(id=self.my_user_id, username=self.bot_username), queued_cmd)
                                print(f"✅ تم تنفيذ أمر من Dashboard: {queued_cmd}")
                                crash_count = 0
                            except Exception as e:
                        ...
                        await asyncio.sleep(1)  # التحقق كل ثانية
                        
                    except asyncio.CancelledError:
                        print("🛑 توقفت حلقة Dashboard commands")
                        raise
                    except Exception as e:
                        print(f"⚠️ خطأ في حلقة Dashboard commands: {e}")
                        await asyncio.sleep(1)
            except asyncio.CancelledError:
                # ♾️ ما نوقف - نحاول دايماً
                await asyncio.sleep(1)
                ...
                crash_count += 1
                print(f"⚠️ dashboard_integration_loop_commands crashed #{crash_count}: {str(e)[:40]} - سيحاول مرة أخرى...")
                # ♾️ يحاول لا نهائياً - لا يتوقف أبداً
                sleep_time = min(2 * crash_count, 30)
                await asyncio.sleep(sleep_time)
    
    async def continuous_dance_loop(self):
        """حلقة مراقبة بسيطة - مع حماية من الانقطاع"""
        crash_count = 0
        ...
        while True:  # 🛡️ Outer wrapper
            try:
                while True:
                    try:
                        ...
                        try:
                            ...
                            bot_found = any(u.id == self.my_user_id for u, _ in (room_users.content if hasattr(room_users, 'content') else []))
                ...
                crash_count += 1
                print(f"⚠️ continuous_dance_loop crashed #{crash_count}: {str(e)[:40]} - سيحاول مرة أخرى...")
                # ♾️ يحاول لا نهائياً - لا يتوقف أبداً
                sleep_time = min(2 * crash_count, 30)
                await asyncio.sleep(sleep_time)
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        ...
        try:
            room_users_response = await self.highrise.get_room_users()
            for u, _ in (room_users_response.content if hasattr(room_users_response, 'content') else []):
        ...
        try:
            room_users_response = await self.highrise.get_room_users()
            all_users = room_users_response.content if hasattr(room_users_response, 'content') else []
        ...
        # 🔇 رسالة الاتصال محذوفة - اتصال صامت (بدون ظهور الروم)
        
        # 🎭 بدء الرقصة التلقائية المستمرة (بدون توقف)
        async def start_dance():
            while True:
                try:
                    self.dance_task = asyncio.create_task(self.continuous_dance_loop())
                    print(f"🎉 تم بدء حلقة الرقصة المستمرة!")
                    await self.dance_task
                except asyncio.CancelledError:
                    break
                except Exception as e:
                    print(f"⚠️ حلقة الرقصة توقفت: {e}, إعادة التشغيل في 10 ثوان...")
                    await asyncio.sleep(10)
        
        try:
            asyncio.create_task(start_dance())
        except Exception as e:
            print(f"⚠️ فشل في بدء الرقصة: {e}")
        ...
        # 🌐 تشغيل حلقة التحكم من Dashboard
        try:
            asyncio.create_task(self.dashboard_integration_loop())
            print("✅ Dashboard integration loop started")
        except Exception as e:
            print(f"⚠️ خطأ في dashboard_integration_loop: {e}")
    ...
    async def on_user_join(self, user: User, position: Position | AnchorPosition):
        ...
        # ترحيب خاص (دائماً يظهر) أو ترحيب عام (بانتظار 5 ثواني بين كل ترحيب عام)
        should_welcome = False
        if username in special_welcomes:
            should_welcome = True
        elif current_time - self.last_welcome_time > 5.0:
            should_welcome = True
            ...
            while True:
                try:
                    try:
                        ...
                        if "not in room" in error_str:
                            print(f"📍 @{user.username} left the room during emote send, stopping loop")
                            break
    ...
    async def follow_user(self, target_username: str):
        ...
        try:
            ...
            while self.is_following:
                try:
                    room_users_response = await self.highrise.get_room_users()
                    room_users = room_users_response.content if hasattr(room_users_response, 'content') else []
                    target_user = None
    ...
    async def on_chat(self, user: User, message: str) -> None:
            ...
            if target_username not in self.haricler:
                ...
                if target_user:
                    if target_user.id not in self.is_teleporting_dict:
                        ...
                        await self.highrise.chat(f"🌀 بدأ تمرجح @{target_user.username}!")
                        
                        async def swing_task(user_id, username):
                            error_count = 0
                            max_errors = 5
                            try:
                                while self.is_teleporting_dict.get(user_id, False):
                                    try:
                                        kl = Position(random.randint(0, 39), random.randint(0, 29), random.randint(0, 39))
                                        await self.highrise.teleport(user_id, kl)
                                        error_count = 0
                                        await asyncio.sleep(1)
                                    except Exception as e:
                                        error_count += 1
                                        print(f"⚠️ خطأ في المرجحة ({error_count}/{max_errors}): {e}")
                                        if error_count >= max_errors:
                                            print(f"❌ توقف التمرجح لـ {username} بعد {max_errors} أخطاء")
                                            break
                                        await asyncio.sleep(2)
                            except asyncio.CancelledError:
                                pass
                            finally:
                                self.is_teleporting_dict.pop(user_id, None)
                                try:
                                    final_position = Position(1.0, 15, 14, "FrontRight")
                                    await self.highrise.teleport(user_id, final_position)
                                except:
                                    pass
                                print(f"🛑 انتهى التمرجح لـ {username}")
                        
                        self.highrise.tg.create_task(swing_task(target_user.id, target_user.username))
                    else:
                        await self.highrise.chat(f"⚠️ @{target_user.username} بالفعل في التمرجح!")
        ...
        if message.startswith("ضفدع"):
            ...
            if len(parts) >= 2:
                ...
                target_user = next((u for u, _ in room_users if u.username.lower() == target_username), None)
                if target_user:
                    await self.highrise.send_emote("emote-frog", target_user.id)
            return

        # 😄 أمر enthused @اسم - شخص معين يرقص
        if message.lower().startswith("enthused"):
            parts = message.split()
            if len(parts) >= 2:
                target_username = parts[1].replace("@", "").lower()
                room_users = (await self.highrise.get_room_users()).content
                target_user = next((u for u, _ in room_users if u.username.lower() == target_username), None)
                if target_user:
                    await self.highrise.send_emote("idle-enthusiastic", target_user.id)
            return

        # 🧟 أمر زومبي @اسم - شخص معين يرقص
        if message.startswith("زومبي"):
            ...
            if len(parts) >= 2:
                ...
                target_user = next((u for u, _ in room_users if u.username.lower() == target_username), None)
                if target_user:
                    await self.highrise.send_emote("emote-zombierun", target_user.id)
            return

        # 🐍 أمر تسس @اسم - شخص معين يرقص
        if message.startswith("تسس"):
            parts = message.split()
            if len(parts) >= 2:
                target_username = parts[1].replace("@", "").lower()
                room_users = (await self.highrise.get_room_users()).content
                target_user = next((u for u, _ in room_users if u.username.lower() == target_username), None)
                if target_user:
                    await self.highrise.send_emote("emote-snake", target_user.id)
            return
        ...
        if message.lower().startswith("add") and message.lower().endswith("mod"):
            ...
            try:
                ...
                try:
                    print(f"🔧 Attempting to add mod: {target_username} (ID: {target_user_id})")
                    ...
                    print(f"✅ Successfully added mod: {target_username}")
                except Exception as e:
                    print(f"❌ Error adding mod: {type(e).__name__}: {e}")
        ...
        if message.lower().startswith("rem") and message.lower().endswith("mod"):
            ...
            try:
                ...
                try:
                    ...
                    await self.highrise.chat(f"💡 شيله يدوي من التطبيق!")
            except Exception as e:
                await self.highrise.chat(f"❌ Error: {e}")
            return

        # 🌟 أمر إضافة في اي بي - add @username vip
        if message.lower().startswith("add") and message.lower().endswith("vip"):
            if not self.is_owner(user):
                await self.highrise.chat("❌ Only owner can add VIPs!")
                return
            parts = message.split()
            if len(parts) < 3 or not parts[1].startswith("@"):
                await self.highrise.chat("Usage: add @username vip")
                return
            target_username = parts[1][1:].lower()
            ...
            if target_username not in vip_users:
                ...
                save_vip_users()
                await self.highrise.chat(f"✅ @{target_username} has been added to VIP list!")
            else:
                await self.highrise.chat(f"ℹ️ @{target_username} is already a VIP!")
            return

        # 🌟 أمر إزالة في اي بي - rem @username vip
        if message.lower().startswith("rem") and message.lower().endswith("vip"):
            if not self.is_owner(user):
                await self.highrise.chat("❌ Only owner can remove VIPs!")
                return
            parts = message.split()
            if len(parts) < 3 or not parts[1].startswith("@"):
                await self.highrise.chat("Usage: rem @username vip")
                return
            target_username = parts[1][1:].lower()
            
            if target_username in vip_users:
                vip_users.remove(target_username)
                save_vip_users()
                await self.highrise.chat(f"❌ @{target_username} has been removed from VIP list!")
            else:
                await self.highrise.chat(f"ℹ️ @{target_username} is not in VIP list!")
            return

        # 🎨 أمر إضافة مصمم - add @username des
        if message.lower().startswith("add") and message.lower().endswith("des"):
            is_owner_check = self.is_owner(user)
            is_mod = user.username.lower() in self.mod_users
            
            if is_owner_check or is_mod:
                parts = message.split()
                if len(parts) < 3 or not parts[1].startswith("@"):
                ...
                try:
                    ...
                    try:
                        new_priv = RoomPermissions(designer=True)
                        await self.highrise.change_room_privilege(target_user_id, new_priv)
                        ...
                        await self.highrise.chat(f"💡 ارفعه designer يدوي من التطبيق!")
                except Exception as e:
                    await self.highrise.chat(f"❌ Error: {str(e)}")
            else:
        ...
        if message.lower().startswith("rem") and message.lower().endswith("des"):
            ...
            if is_owner_check or is_mod:
                ...
                try:
                    ...
                    try:
                        new_priv = RoomPermissions(designer=False)
                        await self.highrise.change_room_privilege(target_user_id, new_priv)
        ...
        # 🌟 أمر عرض الـ VIP - viplist أو vip
        if message.lower() in ["/viplist", "/vip"]:
            if vip_users:
                vips_display = ", ".join([f"@{v}" for v in sorted(vip_users)])
                await self.highrise.chat(f"🌟 VIP Users: {vips_display}")
            else:
                await self.highrise.chat("🌟 No VIP users yet!")
            return

        # 📍 أمر حفظ منطقة الرقص - معطل
        ...
        if msg.startswith("addswm "):
            # ✅ المالكين يقدرون يستخدموا الأمر مباشرة
            if not self.is_owner(user) and user.username.lower() not in allowed_addswm_users:
                await self.highrise.send_whisper(user.id, "❌ هذا الأمر متاح فقط للمالكين أو المصرح لهم.")
                return
        ...
        if msg.startswith("settele "):
            # السماح فقط لصاحب البوت الأساسي
        ...
        global room_commands
        if len(parts) == 1 and parts[0] in ["تحت", "وسط", "فوق", "vr","dj"]:
            command = parts[0]
        ...
        if len(parts) >= 2 and parts[0] in ["تحت", "وسط", "فوق", "vr","dj"]:
            if not await self.is_user_allowed(user):
        ...
        if msg.startswith("tip "):
            ...
            # 💰 التحقق من الجولد أولاً
            bot_gold = await self.get_bot_gold()
            if bot_gold <= 0:
                await self.highrise.chat(f"❌ البوت لا يملك جولد! المحفظة: {bot_gold} جولد")
                return
            ...
            if parts[1] == "all":
                ...
                if bot_gold < amount:
                    await self.highrise.chat(f"❌ جولد غير كافي! المتاح: {bot_gold} جولد، المطلوب: {amount} جولد")
                    return
                ...
                for u, _ in room_users:
                    ...
                    try:
                        ...
                        if "not enough" in str(e).lower():
                            await self.highrise.chat(f"❌ نفذ الجولد بعد إرسال {sent} تيب!")
                            return
        ...
        if msg == "banlist":
            if not self.is_owner(user):  # 🔒 مخصص للمالكين فقط
                await self.highrise.send_whisper(user.id, "❌ هذا الأمر مخصص للمالكين فقط.")
                return
        ...
        if first_word in reaction_mapping:
            ...
            if len(parts) <= 3 and all(not p.isalpha() or p.lower() == first_word for i, p in enumerate(parts) if i > 0):
                ...
                users_response = await self.highrise.get_room_users()
                users = self.get_content(users_response)
    ...
    async def on_message(self, user_id: str, conversation_id: str, is_new_conversation: bool) -> None:
        try:
            ...
            messages = self.get_content(response)

            if not (messages and len(messages) > 0):
                return
            
            message = messages[0].content.strip() if hasattr(messages[0], 'content') else ""
            ...
            try:
                ...
                print(f"⚠️ خطأ في جلب معلومات المرسل: {e}")
            ...
            # ✅ تحقق أولاً لو الرسالة أمر معروف - لا نرسلها للعام
            known_commands = ["اوامر", "commands", "help", "مساعدة"]
            if message_lower not in known_commands:
...
class RunBot():
    room_id = os.environ.get("ROOM_ID", "691c7146253a9060fe8a2e73")
    bot_token = os.environ.get("API_TOKEN", "10ef90feb20554fa486d606f40d708487d0e2cd3c5669829cc5fe8b236eb5187")
    bot_file = "main"
    bot_class = "MyBot"
    ...
    def run_loop(self) -> None:
        ...
        while True:
            try:
                ...
                # إعداد حلقة الحدث لتشغيل البوت بشكل آمن
                ...
                if self.reconnect_attempts > 1000000:
                    self.reconnect_attempts = 1

                # سرعة إعادة اتصال ثابتة (5 ثواني) لضمان البقاء متصلاً
                delay = 5
                ...
                print("[BOT] Unexpected crash, restarting...")
                time.sleep(5)
